...
class MultiHeadAttention(nn.Module):
    def __init__(self, embedding_dim, head_num):
        ...
        self.qkv_layer = nn.Linear(
            embedding_dim, embedding_dim * 3, bias=False)
        self.out_attention = nn.Linear(
...
class ViT(nn.Module):
    def __init__(self, img_dim, in_channels, embedding_dim, head_num, mlp_dim,
        ...
        self.embedding = nn.Parameter(
            torch.rand(self.num_tokens + 1, embedding_dim))
...
class EncoderBottleneck(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, base_width=64):
        ...
        self.conv3 = nn.Conv2d(width, out_channels,
...
class DecoderBottleneck(nn.Module):
    ...
    def forward(self, x, x_concat=None):
        ...
        if x_concat is not None:
            x = torch.cat([x_concat, x], dim=1)  # x (b, c, H, W)
...
class Encoder(nn.Module):
    def __init__(self, img_dim, in_channels, out_channels, head_num, mlp_dim, block_num, patch_dim):
        ...
        self.conv1 = nn.Conv2d(in_channels, out_channels, kernel_size=7,
                               stride=2, padding=3, bias=False)  # (B, C, H, W,)
        self.norm1 = nn.BatchNorm2d(out_channels)
        self.relu = nn.ReLU(inplace=True)

        self.encoder1 = EncoderBottleneck(
            out_channels, out_channels*2, stride=2)
        self.encoder2 = EncoderBottleneck(
            out_channels*2, out_channels*4, stride=2)
        self.encoder3 = EncoderBottleneck(
            out_channels*4, out_channels*8, stride=2)
    ...
    def forward(self, x):
        ...
        x = rearrange(x, "b (x y) c -> b c x y",
                      x=self.vit_img_dim, y=self.vit_img_dim)
